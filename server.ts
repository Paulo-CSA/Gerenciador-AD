...
const initialDNSZones = [
  { name: "empresa.local", type: "Direta", updateType: "Segura" },
  { name: "1.168.192.in-addr.arpa", type: "Inversa", updateType: "Segura" }
];
...
const initialDNSRecords = [
  ...
  },
  {
    ...
    name: "100",
    type: "PTR",
    value: "srv-dc01-old.empresa.local.",
    ttl: 3600,
...
function readDatabase() {
  try {
    ...
    if (!db.dnsZones) {
      db.dnsZones = initialDNSZones;
      modified = true;
    }
    if (!db.dnsRecords) {
      db.dnsRecords = initialDNSRecords;
      modified = true;
    }
...
// 9.5 DNS Management Endpoints
app.get("/api/ad/dns", (req, res) => {
  try {
    const db = readDatabase();
    res.json({
      zones: db.dnsZones || [],
    ...
    console.error("Erro ao ler DNS:", error);
    res.status(500).json({ error: "Erro ao obter dados do DNS." });
  }
});

app.post("/api/ad/dns/records/create", (req, res) => {
  try {
    const { zoneName, name, type, value, ttl, isStatic, operator } = req.body;
    const db = readDatabase();
    ...
    db.logs.unshift({
      ...
      action: "Criação de Registro DNS",
      targetUser: name,
      details: `Criado registro ${type} para '${name}' na zona ${zoneName} com valor '${value}'`,
      type: "success"
    });
    ...
    res.status(500).json({ error: "Erro ao criar registro DNS." });
  }
});

app.post("/api/ad/dns/records/delete", (req, res) => {
  try {
    const { id, operator } = req.body;
    const db = readDatabase();
    
    db.dnsRecords = db.dnsRecords || [];
    const record = db.dnsRecords.find((r: any) => r.id === id);
    if (!record) {
      return res.status(404).json({ error: "Registro DNS não encontrado." });
    }
    
    db.dnsRecords = db.dnsRecords.filter((r: any) => r.id !== id);
    ...
    db.logs.unshift({
      ...
      timestamp: getBrazilTimestamp(),
      operator: operator || "admin.silva",
      action: "Exclusão de Registro DNS",
      targetUser: record.name,
      details: `Excluído registro ${record.type} de '${record.name}' na zona ${record.zoneName}`,
      type: "warning"
    });
    ...
    res.status(500).json({ error: "Erro ao excluir registro DNS." });
  }
});

app.post("/api/ad/dns/clean-duplicates", (req, res) => {
  try {
    const { operator } = req.body;
    const db = readDatabase();
    db.dnsRecords = db.dnsRecords || [];
    
    const originalCount = db.dnsRecords.length;
    const cleanedRecords: any[] = [];
    const duplicatesRemoved: string[] = [];
    
    // Group records by dynamic key to find identical duplicates
    const seenIdentical = new Set<string>();
    
    // Group records by name & type for finding conflicting mappings (like srv-dc01 pointing to two IPs)
    const aRecordsGrouped: Record<string, any[]> = {};
    const ptrRecordsGrouped: Record<string, any[]> = {};
    
    db.dnsRecords.forEach((rec: any) => {
      const identicalKey = `${rec.zoneName.toLowerCase()}|${rec.name.toLowerCase()}|${rec.type}|${rec.value.toLowerCase()}`;
      if (seenIdentical.has(identicalKey)) {
        duplicatesRemoved.push(`Registro idêntico duplicado: ${rec.name} (${rec.type}) -> ${rec.value} na zona ${rec.zoneName}`);
        return; // Skip/remove identical duplicates
      }
      seenIdentical.add(identicalKey);
      
      if (rec.type === 'A') {
        const conflictKey = `${rec.zoneName.toLowerCase()}|${rec.name.toLowerCase()}`;
        if (!aRecordsGrouped[conflictKey]) {
          aRecordsGrouped[conflictKey] = [];
        }
        aRecordsGrouped[conflictKey].push(rec);
      } else if (rec.type === 'PTR') {
        const conflictKey = `${rec.zoneName.toLowerCase()}|${rec.name.toLowerCase()}`;
        if (!ptrRecordsGrouped[conflictKey]) {
          ptrRecordsGrouped[conflictKey] = [];
        }
        ptrRecordsGrouped[conflictKey].push(rec);
      } else {
        cleanedRecords.push(rec);
      }
    });
    
    // Resolve conflicting A records
    Object.keys(aRecordsGrouped).forEach((key) => {
      const records = aRecordsGrouped[key];
      if (records.length <= 1) {
        cleanedRecords.push(...records);
      } else {
        records.sort((a, b) => {
        ...
        cleanedRecords.push(best);
        
        for (let i = 1; i < records.length; i++) {
          duplicatesRemoved.push(`Mapeamento A conflitante resolvido: Removido IP obsoleto ${records[i].value} para ${records[i].name} (mantido ${best.value})`);
        }
      }
    });
    
    // Resolve conflicting PTR records
    Object.keys(ptrRecordsGrouped).forEach((key) => {
      const records = ptrRecordsGrouped[key];
      if (records.length <= 1) {
        cleanedRecords.push(...records);
      } else {
        records.sort((a, b) => {
        ...
        cleanedRecords.push(best);
        for (let i = 1; i < records.length; i++) {
          duplicatesRemoved.push(`Mapeamento PTR conflitante resolvido: Removido ponteiro obsoleto ${records[i].value} para IP .${records[i].name} (mantido ${best.value})`);
        }
      }
    });
    
    db.dnsRecords = cleanedRecords;
    const removedCount = originalCount - cleanedRecords.length;
    ...
    if (removedCount > 0) {
      db.logs.unshift({
        ...
        details: `Varredura e Scavenging concluído. Removidos ${removedCount} registros de DNS duplicados ou obsoletos.`,
        type: "success"
      });
      writeDatabase(db);
    }
    ...
    console.error("Erro ao executar limpeza do DNS:", error);
    res.status(500).json({ error: "Erro ao executar limpeza de registros duplicados." });
  }
});
